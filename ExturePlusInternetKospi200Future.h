#ifndef __EXTURE_PLUS_KOSPI_200_FUTURE_H__
#define __EXTURE_PLUS_KOSPI_200_FUTURE_H__
...
//(파생A) DRV : A001F, A003F
class ExturePlusKospi200FutureStockInfoRecord
{
    public:
	...
	char upLimitPriceForFutureCircuitBreakers[11];               //선물CIRCUIT_BREAKERS상한가
	char downLimitPriceForFutureCircuitBreakers[11];               //선물CIRCUIT_BREAKERS하한가
...
const int EXTURE_PLUS_KOSPI_200_FUTURE_STOCK_INFO_RECORD_SIZE = sizeof(ExturePlusKospi200FutureStockInfoRecord);
#endif

#if(1)
// KOSPI200지수선물_실시간_UDP(8M)_20140901_2014072501.xls
// 15   K200선물_Polling 데이터  I2014   모든 포트 확인용으로 1분간격으로 제공   20  1분간격    코스피200지수선물체결호가정보
class ExturePlusKoscomLineLKInfo_KOSPI200_FUTURE
{
    public:
        // 001 ~ 010
...
const int EXTURE_PLUS_KOSCOM_LINE_LK_INFO_KOSPI200_FUTURE_SIZE = sizeof(ExturePlusKoscomLineLKInfo_KOSPI200_FUTURE);
#endif
...
//(파생A) DRV : A301F, A303F
class ExturePlusKospi200FutureTradeRecord
{
    public:
	char dataClass[2];            //데이터구분값
	char infoMarketClass[3];            //정보구분값
...
const int EXTURE_PLUS_KOSPI_200_FUTURE_TRADE_RECORD_SIZE = sizeof(ExturePlusKospi200FutureTradeRecord);
#endif
...
//파생 시세 Snapshot (우선호가 5단계)
//(파생A) DRV : B201F, B203F
class ExturePlusKospi200FutureRecoveryQuoteRecord
{
    public:
...
const int EXTURE_PLUS_KOSPI_200_FUTURE_RECOVERY_QUOTE_RECORD_SIZE = sizeof(ExturePlusKospi200FutureRecoveryQuoteRecord);

class ExturePlusKospi200FutureRecoveryRecord
{
    public:
	char dataClass[2];           //데이터구분값
	char infoMarketClass[3];           //정보구분값
	...
	char accmTradeAmount[22];           //누적거래대금
	char lastQuoteTypeAskBid[1];           //최종매도매수구분코드
	ExturePlusKospi200FutureRecoveryQuoteRecord quoteRecord[5];
...
const int EXTURE_PLUS_KOSPI_200_FUTURE_RECOVERY_RECORD_SIZE = sizeof(ExturePlusKospi200FutureRecoveryRecord);
#endif
...
//(파생A) DRV : B601F, B603F
class ExturePlusKospi200FuturePriorityQuoteRecord
{
    public:
	char dataClass[2];        //데이터구분값
	char infoMarketClass[3];        //정보구분값
...
const int EXTURE_PLUS_KOSPI_200_FUTURE_PRIORITY_QUOTE_RECORD_SIZE = sizeof(ExturePlusKospi200FuturePriorityQuoteRecord);
#endif
...
//파생 체결 + 우선호가(우선호가 5단계)
//(파생A) DRV : G701F, G703F
class ExturePlusKospi200FutureTradePriorityQuoteRecord
{
    public:
	char dataClass[2];            //데이터구분값
	char infoMarketClass[3];            //정보구분값
...
const int EXTURE_PLUS_KOSPI_200_FUTURE_TRADE_PRIORITY_QUOTE_RECORD_SIZE = sizeof(ExturePlusKospi200FutureTradePriorityQuoteRecord);
#endif
...
//(장중 공개 보드만 포함)
//(파생A) DRV : H101F, H103F
class ExturePlusKospi200FutureInvestorRecord
{
    public:
	char dataClass[2];                       //데이터구분값
	char infoMarketClass[3];                       //정보구분값
...
const int EXTURE_PLUS_KOSPI_200_FUTURE_STOCK_INVESTOR_RECORD_SIZE = sizeof(ExturePlusKospi200FutureInvestorRecord);
#endif
...
//당일확정치 : 장종료후
//(파생A) DRV : H201F, H203F
class ExturePlusKospi200FutureOpenInterestVolumeRecord
{
    public:
	char dataClass[2];                       //데이터구분값
	char infoMarketClass[3];                       //정보구분값
...
const int EXTURE_PLUS_KOSPI_200_FUTURE_OPEN_INTEREST_VOLUME_RECORD_SIZE = sizeof(ExturePlusKospi200FutureOpenInterestVolumeRecord);
#endif
...
//장종료후
//(파생A) DRV : H301F, H304F
class ExturePlusKospi200FutureAdjustedPriceRecord
{
    public:
	char dataClass[2];                       //데이터구분값
	char infoMarketClass[3];                       //정보구분값
...
const int EXTURE_PLUS_KOSPI_200_FUTURE_STOCK_ADJUSTED_PRICE_RECORD_SIZE = sizeof(ExturePlusKospi200FutureAdjustedPriceRecord);
#endif
...
//(채권A) RPO : M401R
//(파생A) DRV : M401F, M403F
class ExturePlusKospi200FutureMarketOperationScheduleDisclosureRecord
{
    public:
...
const int EXTURE_PLUS_KOSPI_200_FUTURE_MARKET_OPERATION_SCHEDULE_DISCLOSURE_RECORD_SIZE = sizeof(ExturePlusKospi200FutureMarketOperationScheduleDisclosureRecord);
#endif
...
//09:00 1회	
//(파생A) DRV : M701F, M703F
class ExturePlusKospi200FuturePreMarketNegotiatedTradeDataRecord
{
    public:
	char data_type[2];                       //데이터구분값
	char infoMarketClass[3];                       //정보구분값
	...
	char trade_value[9];                       //체결가격
	char trading_volume_by_tick[9];                       //거래량
	char end_of_text[1];                       //정보분배메세지종료키워드
};
const int EXTURE_PLUS_KOSPI200_FUTURE_PRE_MARKET_NEGOTIATED_TRADE_DATA_RECORD_SIZE = sizeof(ExturePlusKospi200FuturePreMarketNegotiatedTradeDataRecord);
#endif
...
//(증권B) KNX : O601X
//(파생A) DRV : O601F, O603F
class ExturePlusKospi200FutureQuantityAllocationRecord
{
    public:
...
const int EXTURE_PLUS_KOSPI_200_FUTURE_QUANTITY_ALLOCATION_RECORD_SIZE = sizeof(ExturePlusKospi200FutureQuantityAllocationRecord);
#endif
...
//실시간
//(파생A) DRV : Q201F, Q203F
class ExturePlusKospi200FutureRealTimeUpperLowerLimitPriceRecord
{
    public:
	char dataType[2];                       //데이터구분값
	char infoMarketClass[3];                       //정보구분값
...
const int EXTURE_PLUS_KOSPI_200_FUTURE_REAL_TIME_UPPER_LOWER_LIMIT_PRICE_RECORD_SIZE = sizeof(ExturePlusKospi200FutureRealTimeUpperLowerLimitPriceRecord);
#endif
...
//(파생A) DRV : V101F, V103F
class ExturePlusKospi200FuturePriceLimitRangeExpansionTriggerRecord
{
    public:
	char data_type[2];                       //데이터구분값
	char infoMarketClass[3];                       //정보구분값
...
const int EXTURE_PLUS_KOSPI200_FUTURE_PRICE_LIMIT_RANGE_EXPANSION_TRIGGER_RECORD_SIZE = sizeof(ExturePlusKospi200FuturePriceLimitRangeExpansionTriggerRecord);
#endif

#if(1)
// (파생A) DRV : A601F, A602F, A603F, A604F, A605F, A606F, A607F, A608F, A609F, A610F, A611F, A612F, A613F, A615F, A616F
// 파생 종목마감
class ExturePlusKospi200FutureStockCloseRecord
{
    public:
        char dataClass[2];                              // 데이터구분값
        char infoMarketClass[3];                        // 정보구분값
...
const int EXTURE_PLUS_KOSPI_200_FUTURE_STOCK_CLOSE_RECORD_SIZE = sizeof(ExturePlusKospi200FutureStockCloseRecord);
#endif
...
//(증권B) KNX : A701X
//(파생A) DRV : A701F, A703F
class ExturePlusKospi200FutureMarketOperationRecord
{
    public:
...
const int EXTURE_PLUS_KOSPI_200_FUTURE_MARKET_OPERATION_RECORD_SIZE = sizeof(ExturePlusKospi200FutureMarketOperationRecord);
#endif

#pragma pack()

#endif // __EXTURE_PLUS_KOSPI_200_FUTURE_H__
